...
start_build_process() {
    ...
    *Start Time:* $(date '+%Y-%m-%d %H:%M:%S %Z')"
    ...
    # Kernel & Toolchain
    git clone --recurse-submodules https://github.com/nekoshirro/platform_kernel_xiaomi_sm8450.git kernel/xiaomi/sm8450 -b ksu-next --depth 1
    git clone https://github.com/Evolution-X-Devices/kernel/xiaomi/sm8450-devicetrees.git kernel/xiaomi/sm8450-devicetrees --depth 1
...
#    popd
    ...
    *Status:* $status_text"
    ...
    local sukisu_warning="⚠️ *This build is using KernelSU-Next by default!*"
